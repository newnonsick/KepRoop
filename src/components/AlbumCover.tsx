...
export function AlbumCover({
    ...
    return (
        <div className={cn("aspect-[4/3] bg-slate-50 relative overflow-hidden group/cover", className)}>
            {coverImageUrl && !showCollage ? (
                ...
                <div className="grid grid-cols-2 grid-rows-2 w-full h-full gap-0.5 bg-slate-100">
                    {previewImageUrls.slice(0, 4).map((url, i) => (
                        <div key={i} className="relative w-full h-full overflow-hidden bg-slate-200">
                            <img
                                src={url}
                ...
                previewImageUrls.length === 1 ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-50/50 to-slate-50">
                        <ImageIcon className="h-12 w-12 text-blue-200" strokeWidth={1.5} />
                    </div>
                )
            )}
        </div>
